...
obj_dist = pulp.LpVariable("ObjDist", lowBound=0.)
...
for node in g.nodes:
    prob += pulp.lpSum([var_dict[i, k] for i, k in g.edges if i == node]) <= 1

# total distance constraint
prob += pulp.lpSum([cost[i, j] * var_dict[i, j] for i, j in g.edges]) - obj_dist - 7. >= 0.
